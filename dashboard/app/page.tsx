...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Charts polling: every 30 seconds
    const historyInterval = setInterval(fetchHistory, 30000)
    // Audits polling: every 60 seconds
  ...
  const uniqueClients = useMemo(() => {
    const clientNames = Array.from(new Set(pages.map(p => p.client).filter(Boolean)))
  ...
  // Filtered audit averages
  const filteredAuditAverages = useMemo(() => {
    if (!selectedClientId) return audits.averages
    // When a client is selected, we'd need to recalculate averages
    // For simplicity, we'll show the global averages with a note
    return audits.averages
  }, [audits.averages, selectedClientId])
    ...
    <div className="container">
      <header className="header">
        <div className="header-row">
          <div>
            <h1>Monitor de Landing Pages</h1>
      ...
      {hasProblems && (
        <div className="alert-banner">
          Atenção: {counts.offline + counts.soft404} página(s) com problema detectado!
        </div>
      )}

      {/* Audit Metrics */}
      <AuditMetrics
        averages={filteredAuditAverages}
