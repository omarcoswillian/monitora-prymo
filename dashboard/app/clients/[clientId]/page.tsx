...
import { ChevronRight, Globe, Activity, AlertTriangle, CheckCircle2, XCircle, Clock, Gauge } from 'lucide-react'
...
export default function ClientDetailPage() {
  ...
  const mergedData = useMemo(() => {
    return clientPages.map(page => {
      const statusEntry = status.find(s => s.name === `[${page.client}] ${page.name}`)
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      online: pagesWithStatus.filter(d => d.status?.statusLabel === 'Online').length,
      offline: pagesWithStatus.filter(d => d.status?.statusLabel === 'Offline').length,
      slow: pagesWithStatus.filter(d => d.status?.statusLabel === 'Lento').length,
      soft404: pagesWithStatus.filter(d => d.status?.statusLabel === 'Soft 404').length,
    }
  }, [mergedData])
  ...
  const clientAuditAverages = useMemo(() => {
    ...
    for (const scores of clientAudits) {
      if (scores) {
        if (scores.performance !== null) { sum.performance += scores.performance; counts.performance++ }
        if (scores.accessibility !== null) { sum.accessibility += scores.accessibility; counts.accessibility++ }
        if (scores.bestPractices !== null) { sum.bestPractices += scores.bestPractices; counts.bestPractices++ }
        if (scores.seo !== null) { sum.seo += scores.seo; counts.seo++ }
      }
    }

    return {
      performance: counts.performance ? Math.round(sum.performance / counts.performance) : null,
      accessibility: counts.accessibility ? Math.round(sum.accessibility / counts.accessibility) : null,
      bestPractices: counts.bestPractices ? Math.round(sum.bestPractices / counts.bestPractices) : null,
  ...
  const bestWorstPages = useMemo(() => {
    const pagesWithScores = mergedData
      .filter(d => d.audit?.audit?.scores?.performance !== null && d.audit?.audit?.scores?.performance !== undefined)
      .map(d => ({
        ...d,
  ...
  const toggleEnabled = async (page: PageEntry & { status?: StatusEntry; audit?: PageAuditEntry }) => {
    try {
  ...
  const handleDelete = async (page: PageEntry & { status?: StatusEntry; audit?: PageAuditEntry }) => {
  ...
  const handleRunAudit = async (page: PageEntry & { status?: StatusEntry; audit?: PageAuditEntry }) => {
  ...
  return (
    <div className="container">
      ...
      <div className="cards">
        ...
        <div className={`card ${stats.soft404 > 0 ? 'card-highlight-danger' : ''}`}>
          <div className="card-icon">
      ...
      {hasProblems && (
        <div className="alert-banner">
          Atencao: {stats.offline + stats.soft404} pagina(s) com problema detectado!
        </div>
      )}
      ...
      {/* Best/Worst Pages */}
      {(bestWorstPages.best || bestWorstPages.worst) && bestWorstPages.best !== bestWorstPages.worst && (
          ...
          <div className="best-worst-grid">
            {bestWorstPages.best && (
              <div className="best-worst-card best-card">
                ...
                <Link href={`/pages/${bestWorstPages.best.id}`} className="best-worst-link">
                  {bestWorstPages.best.name}
                </Link>
                <div className="best-worst-score score-good">{bestWorstPages.best.perfScore}</div>
              </div>
            )}
            {bestWorstPages.worst && bestWorstPages.worst !== bestWorstPages.best && (
              <div className="best-worst-card worst-card">
                <div className="best-worst-label">
                  <Gauge size={16} />
                  <span>Pior Performance</span>
                </div>
                <Link href={`/pages/${bestWorstPages.worst.id}`} className="best-worst-link">
                  {bestWorstPages.worst.name}
                </Link>
                <div className={`best-worst-score ${bestWorstPages.worst.perfScore >= 90 ? 'score-good' : bestWorstPages.worst.perfScore >= 50 ? 'score-ok' : 'score-bad'}`}>
